...
const config = {
  birthdate: '05 30, 2004',
  name: 'Rifa Tanjim Mahi'
};
...
x = setInterval(function() {
  ...
  if (distance > 0) {
    ...
    function openBox() {
      var typedTextElements = document.getElementsByClassName("typedText");
        for (var i = 0; i < typedTextElements.length; i++) {
            typedTextElements[i].style.display = "none";
        }
      if (step === 1) {
...
// type.js
// var string = "Lorem Ipsum is simp. Lorem Ipsum has been the industry's  and more recently with desktop publishing software like Aldus PageMaker including versions ."; /* type your text here */
// var array = string.split("");
// var timer;

// function frameLooper () {
// 	if (array.length > 0) {
// 		document.getElementById("content").innerHTML += array.shift();
// 	} else {
// 		clearTimeout(timer);
// 			}
...
var textNode = document.querySelector(".typedText");
var text = textNode.textContent;
textNode.textContent = '';
 function typer(disText, hidText){   
   var len = hidText.length;
   if(len > 0 ){
     setTimeout(function () {  
       if(hidText[0] != undefined){
          disText.textContent += hidText[0];
       }       
      hidText = hidText.slice(1, len);                    
      if (1 <= len ) {         
        typer(disText, hidText);             
      }           
     }, 100);
   }  
  }
...
const paragraph = document.getElementById("typedText");
